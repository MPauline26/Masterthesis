...
%macro adjustment(libname);

proc sql;
create table final.all_orign_dtl_&libname. as 
select
  a.*, 
  f.period as last_period,
  f.loan_age as last_loan_age,
  f.mths_remng as last_mths_remng,
  f.current_int_rt,
  f.dt_dfct_setlmt,
  f.cd_zero_bal,
...
CREATE TABLE ANZAHL AS 
SELECT SUB, COUNT(*) AS ANZ
FROM (SELECT SUBSTR(id_loan,1,5) AS SUB FROM final.all_orign_dtl_part1)
GROUP BY SUB
ORDER BY 1;

CREATE TABLE ANZAHL2 AS 
SELECT SUB, COUNT(*) AS ANZ
FROM (SELECT SUBSTR(id_loan,1,5) AS SUB FROM final.all_orign_dtl_part2)
GROUP BY SUB
ORDER BY 1;
